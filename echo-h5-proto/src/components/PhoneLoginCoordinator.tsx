...
function Dialog({ continuation, finish }: { continuation: Continuation; finish: (value: Outcome | null) => void }) {
  ...
  return <div className="phone-login-backdrop"><section className="phone-login-dialog" role="dialog" aria-modal="true" aria-labelledby="phone-login-title">
    ...
    {!challenge && <><p>验证前不会显示这个手机号是否已有账号。</p><input aria-label="手机号" autoFocus inputMode="tel" autoComplete="tel" value={phone} onChange={(e) => setPhone(e.target.value)} placeholder="+86 138 0000 0000" /><button disabled={busy || !normalized} onClick={() => void run(async () => { const value = await authApi.createPhoneChallenge(normalized!, continuation, key('send')); setChallenge(value.challengeId); keys.current.delete('send') })}>获取验证码</button></>}
    {challenge && !resolution && <><p>确认完成前，当前匿名账号和资料不会变化。</p><input aria-label="短信验证码" autoFocus inputMode="numeric" autoComplete="one-time-code" value={code} maxLength={6} onChange={(e) => setCode(e.target.value.replace(/\D/g, ''))} /><button disabled={busy || code.length !== 6} onClick={() => void run(async () => { const value = await authApi.verifyPhoneChallenge(challenge, code, key(`verify:${code}`)); setResolution({ kind: value.resolution, token: value.resolutionToken }) })}>验证手机号</button></>}
