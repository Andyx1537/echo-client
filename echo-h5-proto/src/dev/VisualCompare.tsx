/**
 * 🔴 **短命比稿板：在 Chrome 里打开看，不要截进 Cursor 对话。**
 *
 * 这里的每一块都是为「在浏览器里并排看两版」搭的。复用生产组件与生产 CSS，
 * 生产侧一改它可能静默变样——不报错、不挂测试，只是长得不对了。所以：
 *
 *  · **不要**把这里的任何一块当成「还能再跑一次」的设施；
 *  · **不要**为了保住它而拦下生产侧该做的改动；
 *  · **不要**截 PNG 塞回对话。交一条 Vite URL（`?visual=`）让人在 Chrome 打开。
 *
 * 长命的那批（会被反复调回来改参数再看的）在 `src/dev/design/`，规矩相反：
 * **自持样式，不 `@import` 生产样式表、不蹭生产类名**，这样它才不会被正式改动带塌。
 *
 * ------------------------------------------------------------------
 * 🔴 **打开前必读：这个工作区的 Vite HMR 会拿旧模块，而且不报任何错。**
 *
 * 症状是**源码已经改对了，页面却还是老样子**——改动可能是删了一个元素、换了一句文案、
 * 改了一条样式。页面上没有报错，控制台干净，overlay 也不出。
 *
 * HMR 拿旧模块，意味着**Chrome 里看到的可能根本不对应当前代码**。
 ...
 *
 * **怎么处理：** 重启 dev server 再在 Chrome 打开，别在浏览器里反复刷新——
...
function authorHomeWorks(): Work[] {
  ...
  return [
    ...
    },
    {
      ...
      id: 'wk_author_home_4',
      title: '窗台上那盆没浇的',
      excerpt: '叶子黄了一圈，还是没舍得换掉。',
      body: '叶子黄了一圈，还是没舍得换掉。',
      mediaUrl: assetUrl('seed-covers/cover-pet-bowl.jpg'),
    },
    {
      ...base,
      id: 'wk_author_home_5',
      title: '雨停之后的阳台',
      excerpt: '毛巾还搭在椅背上，风已经干了一半。',
      body: '毛巾还搭在椅背上，风已经干了一半。',
      mediaUrl: assetUrl('seed-covers/cover-pet-blanket.jpg'),
    },
    {
      ...base,
      id: 'wk_author_home_6',
      ...
      excerpt: '扣环锈了一点，名字还在。',
      body: '扣环锈了一点，名字还在。',
      mediaUrl: assetUrl('seed-covers/cover-pet-collar.jpg'),
    },
  ]
}
...
function AuthorHomeHold({ kept, from }: { kept: boolean; from?: 'wall' | 'comments' }) {
  ...
  useEffect(() => {
    if (kept && scroller.current) scroller.current.scrollTop = 420
  }, [kept])
  return (
    <div className="vc-page">
          ...
          <div className="works-grid up-works">
            <div className="works-col">
              {works.filter((_, i) => i % 2 === 0).map((work) => (
                <WorkCard key={work.id} work={work} onOpen={() => {}} />
              ))}
            </div>
            <div className="works-col">
...
export default function VisualCompare({ params }: { params: URLSearchParams }) {
  ...
  if (which === 'author-home-hold') {
    return (
      <div className="vc-pair">
        <AuthorHomeHold kept={false} />
        <AuthorHomeHold kept={true} />
      </div>
    )
  }
  ...
  return (
    <div className="vc-index">
      <h1>视觉选型对比页（仅开发模式）</h1>
      <p>🔴 短命板：在 Chrome 里点下面的链接看。不要截图塞回 Cursor 对话。</p>
      <ul>
